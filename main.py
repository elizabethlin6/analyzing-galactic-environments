...
def plot_redshift_vs_size():
    ...
    #plt.plot(line_x, line_y, color = 'transparent', marker = '+', linestyle = 'dashed',
      #  linewidth = 1, markersize = 1, label='The smallest redshift value is 0.007 and The largest redshift value is 1.379')
   
    plt.plot(horiz_val, min, color = 'yellow')
    plt.plot(horiz_val, max, color = 'yellow')
   # plt.legend()
...
def get_min_max_redshift():
    ...
    # x = distance ( correlated directly w/ distance ) ['e.IFD']
    # y = redshift

   # plt.clf()
   # e_IFD = extract_galaxy_data('galaxy.csv', ['e.IFD'])
    
   # x = [e_IFD]
   # y = [redshift]

   # plt.scatter(x, y)
   # plt.plot(max_min_list)
   # plt.plot(x, y)
    #plt.show()sx
   # plt.savefig("max_min.jpeg")
...
#def get_data(gal_number):
#   rmag = extract_galaxy_data('galaxy.csv', ['Rmag'])
#   mu_max = extract_galaxy_data('galaxy.csv', ['mumax'])
#   size_list = get_size(rmag, mu_max)
#  redshift_list = get_redshift()
#  #name_list = extract_galaxy_data("galaxy.csv", ["Nr"])
   # print size_list
   # print redshift_list
   # for i in range(len(size_list)):
   # print "Galaxy", gal_number, "has a size value of", size_list[i], "and redshift value of", redshift_list[i],"."
...
def main():   
    ...
    print 
    print "Problem 2: How fast the universe is expanding based on the red shift value of galaxies?"
    print "Result: Right now, we are working on implementing the galaxy_speed(redshift_column_list)"
    print "function in order to define the speed of galaxies in various categories. Results are TBD - Stay tuned!"
    rs = get_redshift()
    galaxy_speed(rs)
    print "Here are some cool information about redshift values of our galaxies!"
    get_min_max_redshift() 
    print "Problem 3: How does the size of the bands of a galaxy affect the red shift values of a galaxy?"
    print "Result: There is a randomness cannot be determined...?? Check out our graph!"
    plot_redshift_vs_size()
    print 
    print "Problem 4: Find the distinction point and significance between bluer galaxies and redder galaxies."
    print "Result: Has not been revealed yet!"
    get_red_blue_graph()
